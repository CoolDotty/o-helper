...
public class AmdGpuControl : IGpuControl
{
    ...
    {
        _oldGpu ??= ADL2_Overdrive_Caps(_adlContextHandle, _internalDiscreteAdapter.AdapterIndex, out _, out _, out int version) == Adl2.ADL_SUCCESS && version < 8;
        if (_oldGpu == false) return default;
        if (Environment.TickCount64 - _legacyTick < PMLogCacheMs) return _legacy;

        int adapterIndex = _internalDiscreteAdapter.AdapterIndex;
        (int? temp, int? use, float? power) data = default;

        try
        {
            if (ADL2_OverdriveN_Temperature_Get(_adlContextHandle, adapterIndex, 1, out int temperature) == Adl2.ADL_SUCCESS)
            {
                if (temperature > 1000) temperature /= 1000;
                if (temperature > 0 && temperature < 125) data.temp = temperature;
            }

            if (ADL2_OverdriveN_PerformanceStatus_Get(_adlContextHandle, adapterIndex, out ADLODNPerformanceStatus status) == Adl2.ADL_SUCCESS
                && status.iGPUActivityPercent >= 0 && status.iGPUActivityPercent <= 100)
                data.use = status.iGPUActivityPercent;

            if (ADL2_Overdrive6_CurrentPower_Get(_adlContextHandle, adapterIndex, 0, out int power) == Adl2.ADL_SUCCESS)
            {
                float watts = power / 256f;
